...
function statusClass(s){const t=String(s).toLowerCase();if(t.includes("delivered"))return"status-delivered";if(t.includes("in transit"))return"status-transit";if(t.includes("pending"))return"status-pending";if(t.includes("out for delivery"))return"status-out";return"status-unknown"}
function renderResults(rows){const tbody=resultsTable.querySelector("tbody");tbody.innerHTML="";rows.forEach(r=>{const tr=document.createElement("tr");const tdId=document.createElement("td");tdId.textContent=r.id;const tdStatus=document.createElement("td");const sb=document.createElement("span");sb.className="status-badge "+statusClass(r.status);sb.textContent=r.status;tdStatus.appendChild(sb);const tdOrigin=document.createElement("td");tdOrigin.textContent=r.origin;const tdDest=document.createElement("td");tdDest.textContent=r.destination;const tdUpd=document.createElement("td");tdUpd.textContent=r.updated;tr.append(tdId,tdStatus,tdOrigin,tdDest,tdUpd);tbody.appendChild(tr);});trackResults.hidden=rows.length===0;}
...
async function loadClients(){try{const r=await fetch("assets/data/clients.json");const data=await r.json();const track=document.getElementById("clients-track");if(!track)return;track.innerHTML="";const items=data.clients;items.forEach(c=>{const name=String(c.name||"");const letter=name.replace(/^Client\s+/i,"").trim().toLowerCase();const slug="client-"+(letter||name.toLowerCase().replace(/\s+/g,"-"));const a=document.createElement("a");a.className="badge";a.href="clients.html#"+slug;a.setAttribute("aria-label",name);const img=document.createElement("img");img.src=c.logo;img.alt=name;img.width=44;img.height=44;img.loading="lazy";img.style.borderRadius="6px";img.onerror=()=>{a.textContent=(name||"").slice(0,1);};a.appendChild(img);track.appendChild(a);});track.innerHTML+=track.innerHTML;}catch{const track=document.getElementById("clients-track");if(track){["a","b","c","d","e","f","g","h","i","j"].forEach(letter=>{const a=document.createElement("a");a.className="badge";a.href="clients.html#client-"+letter;a.textContent=letter.toUpperCase();track.appendChild(a);});track.innerHTML+=track.innerHTML;}}}
function initClientPage(){const list=document.getElementById("clients-list");if(!list)return;const hash=String(location.hash||"").replace("#","");const params=new URLSearchParams(location.search);const target=hash||String(params.get("client")||"");const cards=list.querySelectorAll(".client-card");let found=false;cards.forEach(card=>{const name=String(card.dataset.name||"").toLowerCase();const id=String(card.id||"").toLowerCase();const match=target&&(name===target.toLowerCase()||id===target.toLowerCase());card.classList.toggle("highlight",!!match);if(match&&!found){card.scrollIntoView({behavior:"smooth",block:"center"});found=true;}});const toTop=document.getElementById("to-top");if(toTop){toTop.addEventListener("click",e=>{e.preventDefault();window.scrollTo({top:0,behavior:"smooth"});});}}
