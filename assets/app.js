...
if(inqForm&&inqStatus){inqForm.addEventListener("submit",e=>{e.preventDefault();const d=new FormData(inqForm);const name=String(d.get("name")||"").trim();const email=String(d.get("email")||"").trim();const phone=String(d.get("phone")||"").trim();const message=String(d.get("message")||"").trim();if(!name||!email||!phone||!message){inqStatus.textContent="Please fill in all fields.";return;}inqStatus.textContent="Inquiry submitted. Our team will contact you.";inqForm.reset();});}
...
if(inqForm&&inqStatus){inqForm.addEventListener("submit",async e=>{e.preventDefault();const d=new FormData(inqForm);const name=String(d.get("name")||"").trim();const email=String(d.get("email")||"").trim();const phone=String(d.get("phone")||"").trim();const message=String(d.get("message")||"").trim();if(!name||!email||!phone||!message){inqStatus.textContent="Please fill in all fields.";return;}await sendNotify("inquiry",{name,email,phone,message});inqStatus.textContent="Inquiry submitted. Our team will contact you.";inqForm.reset();});}
...
async function loadClients(){try{const r=await fetch("assets/data/clients.json");const data=await r.json();const track=document.getElementById("clients-track");if(!track)return;track.innerHTML="";data.clients.forEach(c=>{const span=document.createElement("span");span.className="badge";const img=document.createElement("img");img.src=c.logo;img.alt=c.name;img.width=28;img.height=28;img.style.borderRadius="50%";img.onerror=()=>{span.textContent=c.name;};span.appendChild(img);const label=document.createElement("span");label.textContent=c.name;span.appendChild(label);track.appendChild(span);});track.innerHTML+=track.innerHTML;}catch{const track=document.getElementById("clients-track");if(track){["Client A","Client B","Partner X","Partner Y"].forEach(n=>{const s=document.createElement("span");s.className="badge";s.textContent=n;track.appendChild(s);});track.innerHTML+=track.innerHTML;}}}
loadClients();