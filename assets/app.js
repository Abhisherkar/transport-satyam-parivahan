...
async function loadClients(){try{const r=await fetch("assets/data/clients.json");const data=await r.json();const track=document.getElementById("clients-track");if(!track)return;track.innerHTML="";const items=data.clients.slice(0,8);items.forEach(c=>{const span=document.createElement("span");span.className="badge";const img=document.createElement("img");img.src=c.logo;img.alt=c.name;img.width=44;img.height=44;img.style.borderRadius="50%";img.onerror=()=>{span.textContent="";};span.appendChild(img);track.appendChild(span);});track.innerHTML+=track.innerHTML;}catch{const track=document.getElementById("clients-track");if(track){Array.from({length:8}).forEach(()=>{const s=document.createElement("span");s.className="badge";s.textContent="";track.appendChild(s);});track.innerHTML+=track.innerHTML;}}}
loadClients();
